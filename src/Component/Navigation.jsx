...
const {con,logs,OrderRemove} = useContext(AppContext)
    ...
    <>
      <div className={styles.navContainer}>
        ...
        <div className={styles.menuContainer}>
          {menudata?.map((elem) => (
            <div key={elem.name} className={styles.dropdown}>
              <h5>
                   {elem.name === "STORE"? <Link className={styles.links} to="/store">{elem.name}</Link> : <Link className={styles.links} to="/">{elem.name}</Link>}  { elem.drop &&  <IoMdArrowDropdown color="#4296CB" />}
              </h5>
        ...
        <div className={styles.bagsearch}>
         ...
         <Link className={styles.links} to="/cart"> <MdOutlineShoppingBag size={"30px"}/> </Link>
         <p style={{color:"#4293C9"}}>{con.qty===0? "":con.qty}</p>
